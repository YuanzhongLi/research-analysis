...
def alignment_analyze(snap_shot, title, height, \
  ...
  if create_pic:
    ...
    h = 0
    for byte, type, _, hidden_class, data in snap_shot:
      section_num = byte // 8
      hs = section_num // 8
      re = section_num  % 8
      key = get_key(type, hidden_class, byte)
      ...
      if is_target:
        if key in base_dict:
          ...
          compressed_bits = byte * 8 - compression_bits
          ...
          base_dict[key] = data
          total_bits += 8 * byte
          hidden_class_compression_bits += 8 * byte
          meta_class_compression_bits += 8 * byte
      else:
        total_bits += 8 * byte
        hidden_class_compression_bits += 8 * byte
        meta_class_compression_bits += 8 * byte
  ...
  return hidden_class_compressed_bits // 8, meta_class_compressed_bits // 8, total_compressed_bits // 8
...
for i, snap_shot in enumerate(al_snap_shots):
  ...
  # preprocessing
  total_bytes, hidden_class_bytes, meta_class_bytes, \
  dict, targetCellTypeDict, alignment_height = alignment_preprocessing(snap_shot, used_cs, detail=create_pic)

  # analyze
  al_save_pic_path = "./results/{0}/pic".format(benchmark)
  al_save_pic_title = "{0}-alignment{1}".format(benchmark, i+1)
  hidden_class_compressed_bytes, meta_class_compressed_bytes, total_compressed_bytes = \
  alignment_analyze(snap_shot, al_save_pic_title, alignment_height, targetCellTypeDict, al_save_pic_path, create_pic)

  # print compression results
  print_compression_results(total_compressed_bytes, hidden_class_compressed_bytes, hidden_class_bytes, meta_class_compressed_bytes, meta_class_bytes, total_bytes)
