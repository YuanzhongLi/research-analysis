...
def create_line_graph(byte_data, compressed_ratio_data, xlabel, graph_path):
  x_size =  byte_data.shape[0]

  # byte line graph
  total_bytes = byte_data[:, 0]
  total_compressed_bytes = byte_data[:, 1]
  ...
  l1,l2,l3,l4 = "total", "total_compressed", "hidden", "meta"
  ...
  ax.plot(x, total_compressed_bytes, color=c2, label=l2)
  ...
  # ratio line graph
  total_combressed_ratio = compressed_ratio_data[:, 0]
  hidden_ratio = compressed_ratio_data[:, 1]
  meta_ratio = compressed_ratio_data[:, 2]
  total_combressed_ratio_average = np.average(total_combressed_ratio)
  hidden_ratio_average = np.average(hidden_ratio)
  meta_ratio_average = np.average(meta_ratio)
  x = np.arange(1, x_size+1)

  c1,c2,c3 = "blue","green","red"
  l1,l2,l3 = "total_compressed", "hidden", "meta"
  ...
  #ax.set_xlim([1, x_size])
  ax.set_ylim([0, 100])
  ax.plot(x, total_combressed_ratio, color=c1, label=l1)
  ...
  return np.array([total_combressed_ratio_average, hidden_ratio_average, meta_ratio_average])


# create_line_graph(byte_data, compressed_ratio_data, "GC")
...
for i, benchmark in enumerate(benchmarks):
  ...
  byte_data, compressed_ratio_data = read_file(file_path)
  averages = create_line_graph(byte_data, compressed_ratio_data, "GC", graph_path)
...
def create_bar_graph(average_ratios, graph_path):
  ...
  plt.savefig("{0}/compressed-ratio.png".format(graph_path))
