...
from cell_types import CellTypeDict, is_hidden_class, num2color
from utils import get_key
from LZC_algorithm import compress, compress_8byte, compress_64byte
...
def alignment_preprocessing(snap_shot, USED_COLORS, detail=True):
  ...
  for byte, type, _, hidden_class, _ in snap_shot:
    total_bytes += byte
    object_nums[type] += 1
  ...
  print_total(total_bytes, total_types)
  print_compress_target(compress_target_bytes, total_bytes, compress_target_types)
  print_compress_hidden_target(compress_hidden_class_target_types, total_bytes, compress_hidden_class_target_bytes)
  print_compress_meta_target(compress_meta_class_target_types, total_bytes, compress_meta_class_target_bytes)

  if detail:
    print_objects(object_nums, object_bytes, total_bytes)
    print_target_objects(dict)

  return total_bytes, hidden_class_bytes, meta_class_bytes, \
